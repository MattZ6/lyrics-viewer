  ...
  return useMemo(() => {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (segment.type === 'marker') {
        ...
        const nextMarkerIndex = segments.slice(i + 1).findIndex(s => s.type === 'marker')
        const nextMarker = nextMarkerIndex !== -1
          ? segments.slice(i + 1 + nextMarkerIndex).find(s => s.type === 'lyric')
          : null
